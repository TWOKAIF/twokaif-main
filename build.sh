...
# ─── INDEX.HTML ────────────────────────────────────────────
{
  ...
  echo '</head>'
  echo '<body>'
  # Путь по главной: знакомство → доверие → выбор услуги → доказательства
  # → актуальные предложения → полное портфолио → ответы → контакт.
  for f in 01_Глобальные-стили 02_Прелоадер 03_Навигация 04_Главный-экран 05_Обо-мне 06_Бегущая-полоска-клиентов 09_Цены 08_Кейсы 04b_Акция-баннер 11_Портфолио-обложки 12_Портфолио-афиши 13_Победные-презентации 10_Частые-вопросы 14_Футер 15_Куки-баннер; do
    echo
    echo "<!-- ═══ $f ═══ -->"
    cat "$BLOCKS/${f}.html"
  done
