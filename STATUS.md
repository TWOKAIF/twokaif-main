...
## 🆕 2026-07-14 — постоянные ссылки для Instagram (ЛОКАЛЬНО, ЖДУТ ДЕПЛОЙ)
...
Страницы встроены в `build.sh`, поэтому не потеряются при будущей пересборке сайта. Проверены все 6 адресов: HTTP 200, мобильный экран 428×926, большой экран 1440×900, переполнений и ошибок в браузере нет. В прод пока не выкладывались.
