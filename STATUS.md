...
## 🆕 2026-07-15 — новый блок цен и раскрытия (ЛОКАЛЬНО, НЕ ЗАДЕПЛОЕНО)
...
Проверено на мобильном экране 428×926: все 8 карточек открываются по одной, цены не пересекаются с названиями, кнопки видны, ошибок в консоли нет. `dist/` пересобран. На сервер изменения не загружались.
